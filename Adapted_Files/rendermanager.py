...
class RenderManager(abr_scenes.BaseSceneManager):
    ...
    def __init__(self, unit_conversion=bu_to_mm, used_own_scenarios=False):
        ...
        self.used_own_scenarios=used_own_scenarios

    #additionally added
    #######################################################################################
    def compute_obj_center_on_screen(self, obj, camera):
        scene = bpy.context.scene
        loc=obj["bpy"].matrix_world.to_translation()
        co_2d = bpy_extras.object_utils.world_to_camera_view(scene, camera, loc)
        #co_2d[2] is negative that object is behind camera
        ...
        center=(co_2d.x * render_size[0], co_2d.y * render_size[1])
        #pixel coordinates can't be outside of image [0,width) x [0,height)
                
        center_visible=True
        if co_2d[2]<0 or co_2d[0]<0 or co_2d[1]<0 or co_2d[0]>=1 or co_2d[1]>=1:
            center_visible=False
            
        return center_visible, center
    #######################################################################################
    ...
    def postprocess(self, dirinfo, base_filename, camera, objs, zeroing, **kwargs):
        ...
        """
        print("Begin postprocess",file=sys.stderr)
        ...
        for obj in objs:
            #aditionally added
            ###############################################################################################
            center_visible, center=self.compute_obj_center_on_screen(obj, camera)
            obj["center"]=center
            obj["center_visible"]=center_visible
            ###############################################################################################
    ...
    def setup_compositor(self, objs, **kw):
        ...
        self.compositor = abr_nodes.CompositorNodesOutputRenderedObjects(self.used_own_scenarios)
    ...
    def build_render_result(self, obj, camera, zeroing, visibility_from_mask: bool = False):
        ...
        if self.used_own_scenarios:
            render_result_gl = PoseRenderResult(
                ...
                #aditionally added
                #############################################################################################
                n_not_visible= obj['n_pix_not_visible'],
                n_occluded=obj['n_pix_occluded'],
                n_not_visible_and_occluded=obj['n_pix_not_visible_and_occluded'],
                n_pixels=obj['n_pix'],
                center_visible=obj['center_visible'],
                center=obj['center']
                #############################################################################################
                )
        else:
