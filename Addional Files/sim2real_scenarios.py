...
class Sim2Real_Scenarios_Configuration(abr_scenes.BaseConfiguration):
    ...
    def __init__(self):
        ...
        self.add_param('scenario_setup.synthetic_sample', 0, 'Synthetic sample to reproduc')
        #######################################################################################
...
class Sim2Real_Scenarios(interfaces.ABRScene):
    ...
    def generate_dataset(self):
        ...
        while scn_counter < self.config.dataset.scene_count:
            ...
            synthetic_sample=self.config.scenario_setup.synthetic_sample * self.config.dataset.scene_count + scn_counter
            scenarios.Objects(synthetic_sample).randomize_object_transforms([x['bpy'] for x in self.objs], [x['bpy'] for x in self.distractors])
...
#change_objects_loc_rot_here
            ...
            for i_cam, cam_str in enumerate(self.config.scene_setup.cameras):
                ...
                for view_counter, cam_loc in enumerate(cam_locations):
                    ...
                    scenarios.Camera(synthetic_sample).randomize_camera_transforms(cam, [x['bpy'] for x in self.objs], [x['bpy'] for x in self.distractors])
