...
class CompositorNodesOutputRenderedObjects():
    ...
    def __init__(self, used_own_scenario=False):
        ...
        self.objs = []
        self.scene = None
        self.used_own_scenario=used_own_scenario
    ...
    def __extract_pathspec(self):
        ...
        ###################################################################################
        """
        if self.used_own_scenario:
            self.path_mask_per_part = self.dirinfo.images.mask_per_part[len(prefix) + 1:]
            self.path_mask_per_part = os.path.join(self.path_mask_per_part, '')
        """
        ###################################################################################
    ...
    def setup_nodes(self, objs: list, scene: bpy.types.Scene = bpy.context.scene, **kw):
        ...
        for i, obj in enumerate(objs):
            ...
            """
            if self.used_own_scenario:
                render_layer_per_object=nodes.new('CompositorNodeRLayers')
                render_layer_per_object.layer='layer_'+obj['bpy'].name
                
                composite=nodes.new('CompositorNodeComposite')
                tree.links.new(render_layer_per_object.outputs['Image'], composite.inputs['Image'])
                
                n_id_mask_per_part = nodes.new('CompositorNodeIDMask')
                n_id_mask_per_part.index = obj['bpy'].pass_index
                n_id_mask_per_part.use_antialiasing = True
                tree.links.new(render_layer_per_object.outputs['IndexOB'], n_id_mask_per_part.inputs['ID value'])
                
                mask_name = f"MaskPerObject{i:03}"
                n_output_file.file_slots.new(mask_name)
                s_obj_mask = n_output_file.file_slots[mask_name]
                s_obj_mask.use_node_format = True
                tree.links.new(n_id_mask_per_part.outputs['Alpha'], n_output_file.inputs[mask_name])
                self.sockets[f"s_obj_mask_per_part{obj['id_mask']}"] = s_obj_mask
            """
            ###################################################################################
    ...
    def setup_pathspec(self, dirinfo, render_filename: str, objs: dict, scene: bpy.types.Scene = bpy.context.scene):
        ...
        for obj in objs:
            ...
            """
            if self.used_own_scenario:
                self.sockets[f's_obj_mask_per_part{obj["id_mask"]}'].path = os.path.join(
                    self.path_mask_per_part, f'{self.base_filename}{obj["id_mask"]}.png####')
            """
            ###################################################################################
    ...
    def postprocess(self):
        ...
        for obj in self.objs:
            ...
            """
            if self.used_own_scenario:
                fname_mask_per_part = os.path.join(
                   self.dirinfo.images.mask_per_part, f'{self.base_filename}{obj["id_mask"]}.png{frame_number_str}')
                os.rename(fname_mask_per_part, fname_mask_per_part[:-4])
                # store name of mask file into dict of corresponding obj
                # TODO: not sure is good to modify the dict but I like more than the list of fname_masks
                obj['fname_mask_per_part'] = fname_mask_per_part[:-4]
            """
            ###################################################################################
